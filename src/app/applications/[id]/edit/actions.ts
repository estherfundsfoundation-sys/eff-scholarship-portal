...
import {ALL_ANSWER_FIELDS,ALL_UPLOAD_KINDS} from "@/lib/application-fields";
...
export async function saveApplication(formData:FormData){
  ...
  const answers:Record<string,string>={};for(const field of ALL_ANSWER_FIELDS)answers[field]=String(formData.get(field)??"").trim();
  ...
  for(const kind of ALL_UPLOAD_KINDS){

...
export async function requestNationalStaffSignInLink(formData:FormData){
  ...
  try{
    ...
    const url=new URL("/auth/secure-link",origin);
...
export async function signUp(formData:FormData){
  ...
  if(generated.error){
    ...
    const resetUrl=new URL("/auth/secure-link",origin);
  ...
  const verifyUrl=new URL("/auth/secure-link",origin);

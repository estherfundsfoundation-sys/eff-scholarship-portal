...
function trustedRequestOrigin(requestHeaders:Headers){
  ...
  try{
    ...
    if(host==="mentor.estherfundsfoundation.org"||host==="beyond.estherfundsfoundation.org"||host==="portal.estherfundsfoundation.org"||host==="eff-futurelink.vercel.app"||host==="eff-beyond.vercel.app"||(host.startsWith("eff-futurelink-")||host.startsWith("eff-beyond-"))&&host.endsWith(".vercel.app"))return parsed.origin;
  }catch{}
  return configured;
}
...
export async function signUp(formData:FormData){
  ...
  const beyond=origin.includes("beyond.estherfundsfoundation.org")||origin.includes("eff-beyond");
  const accountName=futureLink?"EFF FutureLink":beyond?"EFF Beyond":"Esther Funds Foundation Portal";
  const admin=createAdminClient();
  ...
  if(generated.error){
    ...
    const {error:deliveryError}=await getResend().emails.send({from:emailFrom,to:email,subject:`Finish setting up your ${accountName} account`,html:`<div style="font-family:Arial,sans-serif;line-height:1.6;color:#2d1748"><h1 style="color:#42127F">Finish setting up your ${futureLink?"FutureLink":beyond?"Beyond":"portal"} account</h1><p>An earlier registration attempt started an account for this email. Use the secure button below to verify your inbox and create your password.</p><p><a href="${resetUrl.toString()}" style="display:inline-block;background:#42127F;color:#fff;padding:12px 20px;border-radius:8px;text-decoration:none;font-weight:700">Finish my account</a></p><p>Use only the newest email. Questions? nationals@estherfundsinc.org</p></div>`,text:`Finish setting up your ${accountName} account: ${resetUrl.toString()}\n\nUse only the newest email.`});
  ...
  const {error:deliveryError}=await getResend().emails.send({from:emailFrom,to:email,subject:`Verify your ${accountName} account`,html:`<div style="font-family:Arial,sans-serif;line-height:1.6;color:#2d1748"><h1 style="color:#42127F">Verify your ${futureLink?"FutureLink":beyond?"Beyond":"portal"} account</h1><p>Welcome to ${accountName}. Verify your email address to securely enter your account.</p><p><a href="${verifyUrl.toString()}" style="display:inline-block;background:#42127F;color:#fff;padding:12px 20px;border-radius:8px;text-decoration:none;font-weight:700">Verify my email</a></p><p>Use only the newest email. Questions? nationals@estherfundsinc.org</p></div>`,text:`Verify your ${accountName} account: ${verifyUrl.toString()}\n\nUse only the newest email.`});
...
export async function requestPasswordReset(formData:FormData){const email=String(formData.get("email")??"").trim().toLowerCase();const requestHeaders=await headers();const origin=trustedRequestOrigin(requestHeaders);const futureLink=origin.includes("mentor.estherfundsfoundation.org")||origin.includes("eff-futurelink");const beyond=origin.includes("beyond.estherfundsfoundation.org")||origin.includes("eff-beyond");const accountName=futureLink?"EFF FutureLink":beyond?"EFF Beyond":"Esther Funds Foundation Portal";try{const admin=createAdminClient();const {data,error}=await admin.auth.admin.generateLink({type:"recovery",email});if(error)throw error;const tokenHash=data.properties?.hashed_token;if(!tokenHash)throw new Error("Recovery token was not generated");const resetUrl=new URL("/auth/confirm",origin);resetUrl.searchParams.set("token_hash",tokenHash);resetUrl.searchParams.set("type","recovery");resetUrl.searchParams.set("next","/reset-password");const {error:emailError}=await getResend().emails.send({from:emailFrom,to:email,subject:`Reset your ${accountName} password`,html:`<div style="font-family:Arial,sans-serif;line-height:1.6;color:#2d1748"><h1 style="color:#42127F">Reset your ${futureLink?"FutureLink":beyond?"Beyond":"portal"} password</h1><p>Use the secure button below to create a new password for your ${accountName} account.</p><p><a href="${resetUrl.toString()}" style="display:inline-block;background:#42127F;color:#fff;padding:12px 20px;border-radius:8px;text-decoration:none;font-weight:700">Create my new password</a></p><p>This one-time link expires. If you requested more than one email, use only the newest message.</p><p>Questions? nationals@estherfundsinc.org</p></div>`,text:`Reset your ${accountName} password: ${resetUrl.toString()}\n\nThis one-time link expires. If you requested more than one email, use only the newest message.`});if(emailError)throw emailError;}catch(error){console.error("Password recovery email could not be sent",error);}redirect(`/forgot-password?message=${encodeURIComponent("If that email has an account, a secure reset link is on its way. Use only the newest email; earlier links expire automatically.")}`);}

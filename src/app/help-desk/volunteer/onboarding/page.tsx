...
type Query = {stage?: string; error?: string; saved?: string; score?: string; new?: string};
...
export default async function VolunteerOnboarding({searchParams}: {searchParams: Promise<Query>}) {
  ...
  return <main className="section white"><div className="shell"><div className="section-head"><div><div className="eyebrow">National Help Desk volunteer journey</div><h1>Volunteer Onboarding</h1><p>Application → agreements → six modules → 100% assessment → EFF approval.</p></div><div className="help-desk-training-score"><strong>{profile?.training_score ?? 0}%</strong><span>assessment</span></div></div>{query.error&&<div className="notice error-text"><AlertTriangle/>{query.error}</div>}{query.saved&&<div className="notice"><CheckCircle2/>Application and agreements saved. Continue all six training modules.</div>}

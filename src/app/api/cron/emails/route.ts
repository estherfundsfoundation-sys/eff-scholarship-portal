...
type EmailPayload = {name?: string; claim_url?: string; status?: string; message?: string; item?: string; due_at?: string | null; amount?: number | string | null; acceptance_deadline?: string | null; application_path?: string; acceptance_path?: string; title?:string; deadline?:string|null; scholarship_path?:string;matches?:DigestMatch[]};
function renderMessage(templateKey: string, payload: EmailPayload) {
  ...
  if(templateKey==="weekly_scholarship_matches"){const rows=(payload.matches??[]).slice(0,10).map(item=>`<li style="margin-bottom:16px"><a href="${escapeHtml(`${site}${item.path}`)}"><strong>${escapeHtml(item.title)}</strong></a>${item.amount?` · ${escapeHtml(item.amount)}`:""}${item.deadline?` · due ${escapeHtml(new Date(`${item.deadline}T12:00:00`).toLocaleDateString("en-US"))}`:""}${item.reasons?.length?`<br><span>${escapeHtml(item.reasons.slice(0,2).join(" "))}</span>`:""}</li>`).join("");return{subject:"Your weekly EFF scholarship matches",html:`<p>Hello ${name},</p><p>EFF found current opportunities that may fit the private profile you created.</p><ol>${rows}</ol><p><a href="${site}/scholarships/matches">Review all matches and confirm every requirement with the provider</a>.</p><p>A match is guidance, not a guarantee of eligibility, selection, funding, or an award. You can change weekly email preferences in your matching quiz.</p>`};}

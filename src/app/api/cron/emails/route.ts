...
type SupportResource={title:string;description:string;url:string;provider:string};
type EmailPayload = {name?: string; claim_url?: string; status?: string; message?: string; item?: string; due_at?: string | null; amount?: number | string | null; acceptance_deadline?: string | null; application_path?: string; acceptance_path?: string; title?:string; deadline?:string|null; scholarship_path?:string;matches?:DigestMatch[];resources?:DigestResource[];case_code?:string;school_name?:string;case_path?:string;support_resources?:SupportResource[];other_name?:string;match_score?:number;match_reasons?:string[]};
function renderMessage(templateKey: string, payload: EmailPayload) {
  const site = templateKey.startsWith("futurelink_") ? "https://mentor.estherfundsfoundation.org" : templateKey.startsWith("beyond_") ? "https://beyond.estherfundsfoundation.org" : "https://portal.estherfundsfoundation.org";
  ...
  if(templateKey==="futurelink_application_received")return{subject:"We received your EFF FutureLink profile",html:`<p>Hello ${name},</p><h1 style="color:#42127f">Your FutureLink profile is in verification.</h1><p>Thank you for choosing mentorship with Esther Funds Foundation. Your profile, photo, and evidence remain private while the National Office reviews them.</p><p>You cannot be searched, contacted, or matched before approval. When approved, the FutureLink system—not a manual list—will compare your structured goals, academic and career interests, availability, and communication preferences.</p>${button}`};
  if(templateKey==="futurelink_verified")return{subject:"Your EFF FutureLink profile is verified — complete orientation",html:`<p>Hello ${name},</p><h1 style="color:#42127f">Your FutureLink verification is approved.</h1><p>Open your dashboard to complete the brief required orientation. Once you acknowledge the program purpose, Zoom meeting expectations, and safety boundaries, the automatic matcher can begin looking for a compatible connection.</p>${button}`};
  if(templateKey==="futurelink_onboarding_complete")return{subject:"You are ready for EFF FutureLink matching",html:`<p>Hello ${name},</p><h1 style="color:#42127f">Orientation complete. Matching is active.</h1><p>The FutureLink system is now comparing your verified profile using goals, academic and career fit, support needs, availability, time zone, communication style, cadence, and mentor capacity. We will email you when a connection is ready for private review.</p>${button}`};
  if(templateKey==="futurelink_verification_update")return{subject:"An update is needed for your EFF FutureLink profile",html:`<p>Hello ${name},</p><h1 style="color:#42127f">Your verification has an update.</h1><p>${escapeHtml(payload.message||"Please open your FutureLink dashboard for the next step.")}</p><p>Your profile remains private and outside the matching pool.</p>${button}`};
  if(templateKey==="futurelink_match_proposed")return{subject:"EFF FutureLink found a possible match",html:`<p>Hello ${name},</p><h1 style="color:#42127f">A verified FutureLink connection is ready for your review.</h1><p>The system found a ${escapeHtml(String(payload.match_score??""))}% compatibility fit with <strong>${escapeHtml(payload.other_name??"another verified participant")}</strong>.</p>${payload.match_reasons?.length?`<ul>${payload.match_reasons.slice(0,3).map(reason=>`<li>${escapeHtml(reason)}</li>`).join("")}</ul>`:""}<p>Review the limited profile and accept or decline privately. Declining has no penalty. Contact details are not released unless both people accept and sign the agreement.</p>${button}`};
  if(templateKey==="futurelink_connection_active")return{subject:"Your EFF FutureLink connection is active",html:`<p>Hello ${name},</p><h1 style="color:#42127f">Your mentorship connection is active.</h1><p>Both participants accepted and signed the FutureLink agreement. Open the secure dashboard to see only the contact methods each person approved, plan your first meeting, and use session, rematch, block, and safety controls.</p>${button}`};
  if(templateKey==="futurelink_meeting_plan")return{subject:"Your EFF FutureLink Zoom plan was updated",html:`<p>Hello ${name},</p><h1 style="color:#42127f">Your mentorship meeting rhythm is set.</h1><p>${escapeHtml(payload.message||"Your private Zoom meeting plan was updated.")}</p><p>The Zoom link is only shown inside your secure dashboard.</p>${button}`};
  if(templateKey==="futurelink_meeting_reminder")return{subject:"Your EFF FutureLink meeting is coming up",html:`<p>Hello ${name},</p><h1 style="color:#42127f">Your mentorship meeting is coming up.</h1><p>${escapeHtml(payload.message||"Open your dashboard for your private Zoom plan and next meeting details.")}</p>${button}`};
  if(templateKey==="futurelink_match_ended")return{subject:"Your EFF FutureLink match has ended",html:`<p>Hello ${name},</p><p>${escapeHtml(payload.message||"Your mentorship match has ended and your private contact details are no longer displayed.")}</p>${button}`};
  if(templateKey==="futurelink_delivery_test")return{subject:"FutureLink email system test passed",html:`<p>Hello ${name},</p><h1 style="color:#42127f">The FutureLink email system is working.</h1><p>This controlled launch test confirms that the secure queue, email provider, and standalone mentor-dashboard link are connected.</p>${button}`};
  if(templateKey==="futurelink_session_confirmation")return{subject:"Please confirm your EFF FutureLink session",html:`<p>Hello ${name},</p><h1 style="color:#42127f">A mentor logged a session.</h1><p>Open FutureLink to confirm or dispute the date and duration. Only mentee-confirmed time counts toward verified service hours.</p>${button}`};
  if(templateKey==="futurelink_reminder")return{subject:"A FutureLink step is waiting for you",html:`<p>Hello ${name},</p><p>${escapeHtml(payload.message||"A mentorship step is waiting in your secure FutureLink dashboard.")}</p>${button}`};
  if(templateKey==="futurelink_safety_alert")return{subject:"Protected FutureLink safety review required",html:`<p>Hello ${name},</p><p>${escapeHtml(payload.message||"A protected safety case requires staff review.")}</p>${button}`};
  if(templateKey==="beyond_welcome")return{subject:"Welcome to EFF Beyond — your next chapter is open",html:`<p>Hello ${name},</p><h1 style="color:#42127f">College was one chapter. There is more Beyond.</h1><p>Your EFF Beyond profile is ready. Open My Beyond to join purposeful Circles, explore career and life guidance, connect through FutureLink mentorship, and find ways to give back.</p><p><a href="https://groupme.com/join_group/117295571/z5lprYGT" style="display:inline-block;background:#42127f;color:#fff;padding:12px 18px;text-decoration:none;border-radius:6px">Join the official Beyond GroupMe</a></p><p>Beyond is built for real community—not an endless public feed. Your email address, phone number, and exact address are never displayed in the member directory.</p>${button}<p><strong>Belong. Become. Build. Give back.</strong></p>`};
  ...
  if (templateKey === "help_desk_resource_match") {const resourceList=(payload.support_resources??[]).map(resource=>`<li style="margin-bottom:14px"><strong>${escapeHtml(resource.title)}</strong> <span style="color:#65546f">(${escapeHtml(resource.provider)})</span><br>${escapeHtml(resource.description)}<br><a href="${escapeHtml(resource.url)}">Open official resource</a></li>`).join("");return {subject:`New verified resources for EFF case ${payload.case_code??""}`.trim(),html:`<p>Hello ${name},</p><p>EFF found new verified resources that match the concern reported in case <strong>${escapeHtml(payload.case_code??"")}</strong>${payload.school_name?` at ${escapeHtml(payload.school_name)}`:""}.</p><ul>${resourceList}</ul><p>These are referrals, not promises of eligibility, funding, or approval. Confirm current details with each provider.</p><p><a href="${site}${escapeHtml(payload.case_path??"/help-desk/access")}">Open the National Student Help Desk</a></p>`};}
  ...
  if (templateKey === "application_staff_message") return {subject:payload.status??"An update from Esther Funds Foundation",html:`<p>Hello ${name},</p><p>${escapeHtml(payload.message??"An update is available for your application.").replace(/\n/g,"<br>")}</p>${button}<p>With care,<br><strong>Esther Funds Foundation</strong></p>`};

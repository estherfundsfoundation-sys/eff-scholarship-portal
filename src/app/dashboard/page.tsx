...
export default async function Dashboard() {
  ...
  const [{data: profile}, {data: applications}, {data: requests}, {data: notifications}, {data: reachAmbassador}] = await Promise.all([
    ...
    supabase.from("reach_ambassadors").select("id").eq("user_id", user!.id).eq("active", true).maybeSingle(),
  ]);
  ...
  return <main className="section"><div className="shell">
    ...
    <section className="help-now-banner"><div><div className="eyebrow">FAFSA, school balance, or emergency?</div><h3>Tell us what is putting school at risk.</h3><p>Get a step-by-step plan for financial-aid trouble, emergency expenses, food, housing, transportation, and more.</p></div><Link className="button light" href="/resources">I need help now</Link></section>
    <section className="card portal-checkup-callout" style={{marginTop:24}}><div><div className="eyebrow">Prevent common portal problems</div><h3>Run My Portal Checkup</h3><p>Confirm your account, profile, applications, uploads, requests, and submission status before opening a Tech Desk ticket.</p></div><Link className="button" href="/portal-checkup">Check my portal</Link></section>
    {reachAmbassador&&<section className="card" style={{marginTop:24}}><div className="eyebrow">REACH Ambassador</div><h3>Your campus outreach workspace is ready.</h3><p>Open workshop resources, report activities, and submit photos for National Office review.</p><Link className="button" href="/reach/ambassador">Open ambassador workspace</Link></section>}

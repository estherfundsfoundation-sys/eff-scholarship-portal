...
export async function sendSchoolOutreach(formData:FormData){
  const {user}=await requireHelpDeskStaff();const parsed=sendSchema.safeParse({caseId:String(formData.get("caseId")??""),departmentEmail:String(formData.get("departmentEmail")??""),sourceUrl:String(formData.get("sourceUrl")??""),subject:String(formData.get("subject")??""),body:String(formData.get("body")??"")});
...
export async function markSchoolResponse(formData:FormData){
  const {user}=await requireHelpDeskStaff();const caseId=String(formData.get("caseId")??"");if(!z.string().uuid().safeParse(caseId).success)throw new Error("Invalid case.");

...
import {addReachResource, reviewReachActivity, reviewReachAmbassadorProfile, setReachResourceActive} from "./actions";
...
export default async function ReachAdminPage({
  ...
  searchParams:Promise<{error?:string;resource?:string;reviewed?:string;profileReviewed?:string}>;
}) {
  ...
  const pendingProfiles = publicProfiles?.filter((item) => item.status === "pending_review").length ?? 0;
  ...
  return <main className="section white"><div className="shell">
    ...
    <p>Manage the approved ambassador roster, publish workshop tools, and review every public profile and photo before it appears online.</p>
    {params.error && <div className="notice error-text">{params.error}</div>}
    {params.resource && <div className="notice">Resource added.</div>}
    {params.reviewed && <div className="notice">Activity review saved.</div>}
    {params.profileReviewed && <div className="notice">Ambassador profile review saved.</div>}
    <div className="stats admin-stats">
      ...
      <div className="stat"><strong>{pendingProfiles}</strong><span>Profiles awaiting review</span></div>
      <div className="stat"><strong>{pending}</strong><span>Activities awaiting review</span></div>
    </div>
    ...
    <section style={{marginTop:36}}>
      <div className="eyebrow">Public directory review</div><h3>Ambassador profiles</h3>
      <p>Only publish profiles that the ambassador submitted with consent. Private email addresses and workspace activity remain hidden.</p>
      {publicProfiles?.length ? publicProfiles.map((item) => {
        ...
        return <article className="card" key={item.ambassador_id} style={{marginTop:14}}>
          ...
          <p className="muted">{item.private_photo_path ? "Profile photo submitted for private review." : "No profile photo submitted."}</p>
          <form action={reviewReachAmbassadorProfile} className="stack">
            <input type="hidden" name="ambassadorId" value={item.ambassador_id}/>
            <label>Review note<textarea name="reviewNote" rows={3} defaultValue={item.review_note??""} placeholder="Share any requested changes with the ambassador."/></label>
            <label>Decision<select name="decision" defaultValue={item.status === "pending_review" ? "published" : item.status}><option value="published">Approve and publish profile</option><option value="changes_requested">Request changes</option><option value="private">Keep profile private</option></select></label>
            <button className="button">Save profile review</button>
          </form>
        </article>;
    ...
    <section style={{marginTop:36}}>
      <div className="eyebrow">Private review queue</div><h3>Campus activity submissions</h3>
      {submissions?.length ? submissions.map((item) => {
        ...
        return <article className="card" key={item.id} style={{marginTop:14}}>
          ...
          <form action={reviewReachActivity} className="stack"><input type="hidden" name="submissionId" value={item.id}/><label>Review note<textarea name="reviewNote" rows={3} defaultValue={item.review_note??""}/></label><label>Decision<select name="decision" defaultValue={item.status==="pending_review"?"approved":item.status}><option value="approved">Approve activity (keep photos private)</option><option value="published">Approve and publish submitted photos</option><option value="changes_requested">Request changes</option><option value="not_published">Review and keep private</option></select></label><button className="button">Save review</button></form>
        </article>;

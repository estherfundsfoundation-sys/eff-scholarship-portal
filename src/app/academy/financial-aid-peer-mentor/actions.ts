...
export async function submitFinancialAidPeerMentorAssessment(formData: FormData) {
  ...
  try {
    await getResend().emails.send({
      ...
      html: `<div style="font-family:Arial,sans-serif;line-height:1.65;color:#2b1740"><h1 style="color:#42127F">Course completed</h1><p>Congratulations! You passed the EFF Financial Aid Peer Mentor course with a score of <strong>${score}%</strong>.</p><p>Your certificate code is <strong>${certificateCode}</strong>.</p><p><a href="${certificateUrl}" style="display:inline-block;background:#42127F;color:#fff;padding:12px 20px;border-radius:999px;text-decoration:none;font-weight:700">View my certificate</a></p><p>This certificate recognizes completion of an Esther Funds Foundation peer-navigation curriculum. It is not a federal credential or authorization to act as a financial-aid administrator.</p></div>`,
      text: `Congratulations! You passed the EFF Financial Aid Peer Mentor course with a score of ${score}%. Certificate code: ${certificateCode}. View your certificate: ${certificateUrl}\n\nThis is an EFF course-completion credential, not a federal credential or authorization to act as a financial-aid administrator.`,
    });
  } catch (deliveryError) {

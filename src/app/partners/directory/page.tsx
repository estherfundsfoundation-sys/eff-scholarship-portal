...
export default async function PartnerDirectory({searchParams}:{searchParams:Promise<{q?:string;state?:string}>}){
  ...
  try{
    ...
    let query=admin.from("eff_partner_institutions")
      ...
      .eq("status","active").eq("public_profile",true).order("display_name");
  ...
  return <main className="partner-directory-page">
    <section className="partner-directory-hero"><div className="shell"><Link className="back-link" href="/partners">← College Continuity Partnership</Link><div className="eyebrow">National partner directory</div><h1>Schools committed to <span>stepping in.</span></h1><p>Every public institution below has completed the EFF Continuity Pledge and activated a student-continuity partnership. Partner status is voluntary and free; EFF may review profiles after activation.</p>
    ...
    <section className="section white"><div className="shell">
      ...
      {partners.length?<div className="partner-directory-grid">{partners.map(partner=><article className="partner-school-card" key={partner.id}>
        ...
        <div className="partner-card-designation"><BadgeCheck/>{partner.designation==="institute"?"EFF Institute for Student Continuity":"Every Future Fulfilled Partner Campus"}</div>

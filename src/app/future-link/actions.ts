...
import {futureLinkAgreementVersion,futureLinkCodeVersion} from "@/lib/futurelink";
...
export async function requestFutureLinkRematch(form:FormData){const {user}=await currentUser();const admin=createAdminClient();const matchId=text(form,"matchId",50);const {data:match}=await admin.from("futurelink_matches").select("mentee_id,mentor_id,status").eq("id",matchId).maybeSingle();if(!match||![match.mentee_id,match.mentor_id].includes(user.id))return;await admin.from("futurelink_matches").update({status:"ended",ended_at:new Date().toISOString(),end_reason:"Participant requested a new match"}).eq("id",matchId);await admin.from("futurelink_profiles").update({status:"searching_for_match",updated_at:new Date().toISOString()}).in("user_id",[match.mentee_id,match.mentor_id]);await runFutureLinkMatching(10);revalidatePath("/future-link/dashboard");}

...
export default async function SouthernSchoolsPage({searchParams}:{searchParams:Promise<{state?:string;q?:string}>}){
  ...
  return <main className="applyall"><section className="applyall-workspace"><div className="shell"><span className="applyall-kicker">Southern institution directory</span><h1>Find your starting point.</h1><p className="applyall-lead">Explore sourced colleges and universities across the Southern launch region. “Official application” opens the institution-listed application page. A listing does not mean ApplyAll automation is approved.</p>
    ...
    {error?<div className="applyall-alert">The Southern directory will appear after the ApplyAll database migration is applied.</div>:<><p><strong>{schools.length}</strong> sourced institutions shown{schools.length===500?" (refine your search to see more)":""}.</p><div className="applyall-directory-grid">{schools.map((school)=><article key={school.id}><span>{school.state_code}{school.hbcu?" · HBCU":""}</span><h2>{school.name}</h2><p>{[school.city,school.institution_type,school.public_private].filter(Boolean).join(" · ")}</p><div>{school.application_url?<a className="button outline" href={school.application_url} target="_blank" rel="noopener noreferrer">Official application</a>:<span className="muted">Official application link under verification</span>}</div><small>ApplyAll route: mapping required</small></article>)}</div></>}
    <div className="applyall-footer-actions"><Link className="button outline" href="/apply-everywhere">Back to Apply Everywhere</Link></div>
  </div></section></main>;
}

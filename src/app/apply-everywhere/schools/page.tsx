...
export default async function SouthernSchoolsPage({searchParams}:{searchParams:Promise<{state?:string;q?:string;error?:string}>}){
  ...
  const db=await createClient();let query=db.from("applyall_institutions").select("id,name,city,state_code,institution_type,public_private,hbcu,application_url,eff_member_status,eff_receiver_enabled").eq("active",true).eq("is_demonstration",false).order("name").limit(500);

...
const score=(url,route)=>{const lower=url.toLowerCase();if(/\/(news|events?|jobs?|blog|videos?|tag|category|attachment|press-release|articles?|stories|awards?|degrees?|pathways?|campaigns?)\//i.test(lower)||/[_-]pathways?(?:[./_-]|$)/i.test(lower)||/diabetes-prevention/i.test(lower)||/\/20\d{2}\/\d{1,2}\//.test(lower)||/\.(pdf|docx?|xlsx?)$/i.test(lower))return -100;let points=0;for(const term of route.terms)if(lower.includes(term))points+=term.length+20;return points;};

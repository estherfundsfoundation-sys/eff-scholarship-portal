...
const [csvArg,outputArg,offsetArg="0",limitArg="15"]=process.argv.slice(2);
if(!csvArg||!outputArg)throw new Error("Usage: node tools/discover_official_school_contacts.mjs <HD.csv> <output.json> [offset] [limit]");
...
const schools=rows.filter(row=>get(row,"CYACTIVE")==="1"&&get(row,"POSTSEC")==="1"&&get(row,"HBCU")==="1").map(row=>({unitid:Number(get(row,"UNITID")),name:get(row,"INSTNM"),state:get(row,"STABBR"),website:normalizeUrl(get(row,"WEBADDR"))})).filter(x=>x.website).sort((a,b)=>a.name.localeCompare(b.name)).slice(offset,offset+limit);
...
const score=(url,route)=>{const lower=url.toLowerCase();if(/\/(news|events?|jobs?|tag|category|attachment|press-release)\//i.test(lower)||/\/20\d{2}\/\d{1,2}\//.test(lower)||/\.(pdf|docx?|xlsx?)$/i.test(lower))return -100;let points=0;for(const term of route.terms)if(lower.includes(term))points+=term.length+20;return points;};
function extractContact(page,officialHost,route){
  ...
  const editorialTitle=/\b(archives?|attachment|job|professor|prepares|participates|names|donates|continues|celebrates|announces|workshop|conference|grant|minor|degree program)\b/i.test(titlePlain);
...
fs.writeFileSync(path.resolve(outputArg),JSON.stringify({source:"NCES IPEDS HD2023 plus official institution websites",offset,limit,generated_at:new Date().toISOString(),schools:results},null,2),"utf8");
console.log(`Verified ${results.reduce((n,x)=>n+x.contacts.length,0)} department pages across ${results.length} HBCUs.`);
